...
func RegisterRoutes(rg *gin.RouterGroup, h Handlers, userAuth port.UserAuthService) {
	rg.Use(ErrorHandle())

	auth := Authenticate(userAuth)

	// 회원 인증: register/login/refresh 는 공개, me/logout 은 보호.
	rg.POST("/auth/register", h.UserAuth.register)
	rg.POST("/auth/login", h.UserAuth.login)
	rg.POST("/auth/refresh", h.UserAuth.refresh)
